...
super(形参列表):调用父类的构造器的意思，也可以带参数。

**this()就是调用自己的无参的构造函数，你还记得吗？**

+ 如果首行没有写super，会默认调用父类的空参构造器！*也就是说，相当于会自动给你加上super( )*
+ 一个类至少会有一个super，来创建他的父类，直到调用java.lang.Object类中的空参构造器。
+ <font color='red'>虽然创建子类对象，调用了父类的构造器，但内存中只有new出来的子类对象，没有父类对象，只是将父类的属性和方法加载到内存当中了。</font>
...
+ 多态的显著作用就是定义函数的时候，我写函数体用父类来写，但传入可以传入子类对象，让他的子类对象都能用这个函数，而不必写多个以子类对象为形参的函数。
+ 对象的多态性只适用于方法，不适用于属性。   

<font color='red'>多态是编译时行为还是运行时行为？</font>

+ 运行时行为。在编译阶段不知道要执行什么子类方法。 多态时，父类的方法称为虚拟方法。 

![image-20220311224231084](https://s2.loli.net/2022/03/11/4WjdIpxHErZQbJC.png)

----

Person p2 = new Man();

此时堆内存里有Man的所有属性和方法，但p2只能调用Person的属性和方法。编译时p2只能调用父类的属性和方法。<font color='red'>可以通过强制类型转换（向下转型）。但有风险，可能转到别的子类去了。会出现ClassCaseException异常。</font>



能不能成功向下转型，关键看堆空间中是否加载子类的属性、方法。

#### instanceof关键字

a instancof A：判断对象a是否是类A的实例。如果是，返回true，不是返回false。

**为了避免在向下转型时出现ClassCaseException，向下转型前先进性instanceof判断（这个判断应该是在运行期间判断的）**
